...
## Retrospective release (2026-07-15)

**What shipped:** the project was repositioned from a completed-season dashboard into a portfolio retrospective ("a traceable NBA playoff forecasting experiment"), gated by a claim-eligibility audit performed before any copy was written.

- **Phase 0 (gates all claims):** `scripts/backtest/significance.ts` computes paired bootstrap CIs for every 150-series Brier contrast under both series and season-clustered resampling (`docs/backtest/significance.json`). Verdicts: conclusive vs coinflip (−0.0593), home-team (−0.0246), higher-seed (−0.0218); inconclusive vs SRS-only (−0.0029) and net-rating-only (−0.0042). `docs/parameter-provenance.md` establishes via git history that `defaultModelSettings` predates the backtest harness (2026-05-11 vs 2026-05-14) and was never refit, classifying the 150-series result as descriptive, not out-of-sample. `docs/claims-ledger.md` fixes the supported/qualified/forbidden claim categories and the mechanical featured-content rules (replay = most recent archived Finals; failures = highest-confidence misses by computed Brier loss).
- **Routes:** `/` is a five-section retrospective narrative; `/snapshot` holds the frozen completed-season dashboard behind a terminal-state banner with the scoreboard relabeled as an engineering appendix; `/case-study` (stale) was deleted and permanently redirects (308) to `/`; `/evidence` gained the two-regime comparison table, calibration-slope disclosure (≈0.81 series / 0.82 game), and rule-based default replay; `/lab` unchanged.
- **Claim-compliance edits:** `BacktestSummaryCard` and `/methodology` now disclose the inconclusive rating-baseline contrasts and the 69.3%-vs-70.0% accuracy comparison; AGENTS.md records the claim gate for future agents.

**Verification (run 2026-07-15, this session):** `corepack pnpm verify` → "Model verification passed."; `corepack pnpm build` → 7 routes, all pages static except `/api/live-scoreboard`; `pnpm exec eslint src scripts` → clean (repo-wide `pnpm lint` fails only on pre-existing `.vercel/output/` build artifacts); production smoke test on `next start` confirmed all five routes render their section markers and `/case-study` returns 308 → `/`.

**Open items:** walkthrough recording not produced; push/deploy to Vercel left to the maintainer; raw-HTML archive, JSON state migration, and uncertainty-coverage measurement remain open from the prior audit.
