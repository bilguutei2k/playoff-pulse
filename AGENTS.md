...
- All public claims are gated by `docs/claims-ledger.md`. Statistical significance for the 345-series backtest lives in `docs/backtest/significance.json` (regenerate with `pnpm exec tsx scripts/backtest/significance.ts`); parameter provenance is recorded in `docs/parameter-provenance.md`. Do not add performance claims that are not supported by those documents — in particular, the full model does NOT outperform SRS-only or net-rating-only baselines and must never be framed that way.
...
- It is not a betting product. Modeling inputs — team ratings, player impact, projected minutes, injury statuses, and model weights — remain manual configuration; the model itself does not consume live data at runtime. The reconstruction covers 345 playoff series and 1,929 pregame states (2003–2025), including era-correct Finals home patterns. Rolling-origin evaluation covers 1,675 games and 300 series (2006–2025). Historical availability coverage is zero and injury effects are not claimed. See docs/backtest/methodology.md and docs/claims-ledger.md. It is not calibrated against external benchmarks or production-grade for unsupervised public use.
