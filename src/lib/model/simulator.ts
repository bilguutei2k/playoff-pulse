...
function nextGameForecastFor(
  ...
  const teamAWinProbability = clampProbability(
    probability(teamA, teamB, homeTeamId, settings),
  );
  const homeCourt =
    homeTeamId === teamA.id
  ...
  return {
    ...
    expectedMarginForTeamA: margin(teamA, teamB, homeTeamId, settings),
    teamAWinProbability,
...
export function nextBaselineGameForecast(
  series: Series,
  teamsById: Record<string, Team>,
  settings: ModelSettings,
): GameForecast | null {
  return nextGameForecastFor(
    series,
    teamsById,
    settings,
    baselineProbability,
...
export function simulateBaselineSeriesOutcome(
  series: Series,
  teamsById: Record<string, Team>,
  settings: ModelSettings,
  random: RandomSource,
  ...
  while (winsA < 4 && winsB < 4) {
    ...
    const probabilityA = clampProbability(
      baselineProbability(teamA, teamB, homeTeamId, settings),
    );
...
export function estimateBaselineSeriesProbability(
  series: Series,
  teamsById: Record<string, Team>,
  settings: ModelSettings,
  iterations = settings.simulationIterations,
  ...
  return {
    ...
    winsA: series.winsA,
    winsB: series.winsB,
    nextGame: nextBaselineGameForecast(series, teamsById, settings),
    teamASeriesWinProbability,
    teamBSeriesWinProbability: clampProbability(1 - teamASeriesWinProbability),
    expectedGamesRemaining: solution.expectedGamesRemaining,
    iterations: 0,
    ...
    finalScoreProbabilities: solution.finalScoreProbabilities,
    uncertainty: {
      lower: teamASeriesWinProbability,
      median: teamASeriesWinProbability,
      upper: teamASeriesWinProbability,
      samples: 0,
    },
    scenarioImpacts: [],
    teamStrengthDifference:
